...
<head>
    ...
    <link href="https://cdn.datatables.net/v/dt/jszip-3.10.1/dt-2.0.7/b-3.0.2/b-colvis-3.0.2/b-html5-3.0.2/b-print-3.0.2/datatables.min.css" rel="stylesheet">
...
<header class="px-3 pt-3 pb-1">
    <nav class="navbar bg-gray col-12 mx-auto rounded-3">
        <div class="container-fluid">
            <a class="text-white m-0 col-2 text-decoration-none" href="{{route('route_home')}}"><i class="bi bi-house-door"></i><Span class="ms-2 e">Início</Span></a>
            <form class="d-flex col-6 mx-auto w-50" role="search">
                <input class="form-control me-2 rounded-3" type="search" placeholder="Search Musics" aria-label="Search">
                <button class="btn text-white" type="submit"><i class="bi bi-search"></i></button>
            </form>
            @auth
                <form action="{{ route('logout') }}" method="POST" >
                    @csrf
                    <button class="btn btn-danger w-100 text-white col-2 me-3" type="submit">Logout</button>
                </form>
                <img src="{{asset('storage/user/photos/profilePhoto.webp')}}" href="" class="img-fluid navbar-brand rounded-circle" alt="" width="30" height="24">
            @else
                <a class="text-white col-2 text-end text-decoration-none" href="{{route('login')}}"><i class="bi bi-door-open"></i><span class="mx-2">Login</span></a>
            @endauth
        </div>
    </nav>
</header>
